...
2. equals() 和 == 的区别
3. 可以用 == 比较 String 吗
4. == 在什么情况下比较 String 是对的，什么情况下是不对的
5. String s1 = "abc"; 创建了几个对象
6. String s1; 这样写会创建对象吗
7. 可以用 == 比较包装类型吗？
8. 基本类型和包装类型的区别
9. String 是基本类型还是引用类型
10. String 底层数据结构
11. JDK 9 为何要将 String 的底层实现由 char[] 改成了 byte[] ? 是特定情况下使用 byte[] 还是一直使用？
12. 西文字符和 CJK 字符的最大区别 
13. 为什么 ASCII 里面没有中文？
14. ASCII 字符集的范围是什么
15. 列举几个主流的字符集
16. HashMap 的数据结构？它的键值对在内存中是怎么保存的？查询时间复杂度？使用场景？为什么叫做 HashMap？
17. 一句话介绍 Spring。解决了什么问题？解耦是怎样体现的？ 
18. Spring 当中的事务是怎样实现的？
19. AOP 是怎样实现的？
20. 什么叫做动态代理，这四个字怎么理解？怎么做到的动态代理的？
21. C / Python 可以实现动态代理吗？
22. 静态语言和动态语言的主要区别？
23. 编译是在干什么？
24. 反问
...
4. 什么渠道去学习？
5. 说一下项目？
6. 追着问bug。。。怎么修正的？bug 添加不进去什么原因导致的呢？
8. 一张表，表里面有好多字段，查文章在 10:00 - 11:00 这一个小时有多少个人来访问了我这个文章？（有人访问一遍就会记录一次信息）
8. 数据库里面索引的坏处？
9. Linux 往日志文件写东西权限不足？chmod
10. 你的 Linux 下的根目录下的A文件拷贝到我的根目录下的一个 B 文件夹里面。cp 是机器本身的拷贝，通过什么方法可以把你的操作系统ip和密码拿过来？
10. 泛型？IO 流有几种方式啊？一个图片用什么流？要用字节流。。分为字节流和字符流
11. 场景设计：一个商品加入购物车，购买，弹出微信，支付成功，支付方可以放手，那么输完密码成功那一刻开始，系统去做了什么操作？
