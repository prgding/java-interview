...
function computedEager(fn, options) {
  ...
  watchEffect(() => {
    ...
    ...options,
    flush: (options == null ? void 0 : options.flush) ?? "sync"
  });
...
function onLongPress(target, handler, options) {
  ...
  function onDown(ev) {
    ...
    timeout = setTimeout(
      () => handler(ev),
      (options == null ? void 0 : options.delay) ?? DEFAULT_DELAY
    );
  }
  ...
  const element = customRef((track, trigger) => {
    ...
    return {
      get() {
        var _a;
        track();
        return ((_a = instance == null ? void 0 : instance.proxy) == null ? void 0 : _a.$refs[key]) ?? initialValue;
      },
      set() {
      }
    };
...
function useCycleList(list, options) {
  ...
  function getInitialValue() {
    return toValue((options == null ? void 0 : options.initialValue) ?? toValue(list)[0]) ?? void 0;
  }
...
function useDisplayMedia(options = {}) {
  ...
  async function _stop() {
    var _a;
    (_a = stream.value) == null ? void 0 : _a.getTracks().forEach((t) => t.stop());
    stream.value = void 0;
  }
  function stop() {
    _stop();
...
function useDropZone(target, options = {}) {
  ...
  if (isClient) {
    ...
    const getFiles = (event) => {
      var _a;
      const list = Array.from(((_a = event.dataTransfer) == null ? void 0 : _a.files) ?? []);
...
function useElementByPoint(options) {
  ...
  const cb = () => {
    element.value = toValue(multiple) ? (document2 == null ? void 0 : document2.elementsFromPoint(toValue(x), toValue(y))) ?? [] : (document2 == null ? void 0 : document2.elementFromPoint(toValue(x), toValue(y))) ?? null;
  };
...
function useFetch(url, ...args) {
  ...
  const {
    fetch = defaultWindow == null ? void 0 : defaultWindow.fetch,
    initialData,
    timeout
  } = options;
...
function useFileSystemAccess(options = {}) {
  ...
  const fileMIME = computed(() => {
    var _a;
    return ((_a = file.value) == null ? void 0 : _a.type) ?? "";
  });
  const fileSize = computed(() => {
    var _a;
    return ((_a = file.value) == null ? void 0 : _a.size) ?? 0;
  });
  const fileLastModified = computed(() => {
    var _a;
    return ((_a = file.value) == null ? void 0 : _a.lastModified) ?? 0;
  });
...
function useFullscreen(target, options = {}) {
  ...
  const targetRef = computed(() => unrefElement(target) ?? (document2 == null ? void 0 : document2.querySelector("html")));
...
function useScroll(element, options = {}) {
  ...
  function scrollTo(_x, _y) {
    ...
    if (!_element)
      return;
    (_a = _element instanceof Document ? window2.document.body : _element) == null ? void 0 : _a.scrollTo({
...
function useMouseInElement(target, options = {}) {
  ...
  const targetRef = ref(target ?? (window2 == null ? void 0 : window2.document.body));
...
function usePointerLock(target, options = {}) {
  ...
  if (isSupported.value) {
    useEventListener(document2, "pointerlockchange", () => {
      const currentElement = document2.pointerLockElement ?? element.value;
...
function useTextSelection(options = {}) {
  ...
  const text = computed(() => {
    var _a;
    return ((_a = selection.value) == null ? void 0 : _a.toString()) ?? "";
  });
...
function useTitle(newTitle = null, options = {}) {
  ...
  const title = toRef2(newTitle ?? (document2 == null ? void 0 : document2.title) ?? null);
...
function executeTransition(source, from, to, options = {}) {
  ...
  const trans = typeof options.transition === "function" ? options.transition : toValue(options.transition) ?? identity;
  ...
  return new Promise((resolve) => {
    ...
    const tick = () => {
      ...
      if (Array.isArray(source.value))
        source.value = arr.map((n, i) => lerp(v1[i] ?? 0, v2[i] ?? 0, alpha));
